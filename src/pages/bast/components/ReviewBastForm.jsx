...
const ReviewBastForm = () => {
  ...
  const handleCheckApprover = async () => {
    ...
    if (!/\S+@\S+\.\S+/.test(email)) {
      setErrors((prev) => ({
        ...prev,
        pejabatApproval: "Format email tidak valid",
      }));
      return;
    }
    ...
    try {
      ...
      if (res.ok && userData && ["approval"].includes(userData.role)) {
        ...
        setErrors((prev) => ({
          ...prev,
          pejabatApproval: "Email tidak ditemukan atau bukan approver",
        }));
      ...
      setErrors((prev) => ({
        ...prev,
        pejabatApproval: "Gagal memeriksa email",
      }));
      // Tampilkan modal error
      setModalContent({
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/bast/${idBast}/approve-user`,
        {
          method: "PUT",
          headers: {
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/bast/${idBast}/reject`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      setModalContent({
        title: "BAST",
        message: "BAST berhasil approve dan akan dialihkan ke dashboard.",
        type: "success",
      });
      setShowApproverModal(true);
          ...
          <div className="flex gap-2">
            <input
              ...
              className={`flex-1 border rounded px-3 py-2 ${
                errors.pejabatApproval ? "border-red-500" : ""
              }`}
