// src/pages/bast/components/ReviewBastForm.jsx
...
const ReviewBastForm = () => {
  ...
  const handleCheckApprover = async () => {
    const email = formData.pejabatApproval;
    if (!email) {
      setErrors(prev => ({ ...prev, pejabatApproval: 'Email harus diisi' }));
      return;
    }
    
    // Validasi format email
    if (!/\S+@\S+\.\S+/.test(email)) {
      setErrors(prev => ({ ...prev, pejabatApproval: 'Format email tidak valid' }));
      return;
    }
    ...
    try {
      ...
      if (res.ok && userData && ['approval'].includes(userData.role)) {
        setApproverStatus('found');
        setErrors(prev => ({ ...prev, pejabatApproval: undefined }));
        ...
        setErrors(prev => ({ ...prev, pejabatApproval: 'Email tidak ditemukan atau bukan approver' }));
      ...
      setErrors(prev => ({ ...prev, pejabatApproval: 'Gagal memeriksa email' }));
      // Tampilkan modal error
      setModalContent({
  ...
  const validateForm = (isApproval = true) => {
    ...
    if (!formData.pejabatApproval) {
      newErrors.pejabatApproval = 'Pejabat Approval wajib diisi';
    } else if (!/\S+@\S+\.\S+/.test(formData.pejabatApproval)) {
      newErrors.pejabatApproval = 'Format email tidak valid';
    } else if (approverStatus !== 'found') {
      newErrors.pejabatApproval = 'Harap cek validitas email approver terlebih dahulu';
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bast/${idBast}/approve-user`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          userEmail,
          pejabatApproval: formData.pejabatApproval,
          note: formData.notes
        })
      });
  ...
  const handleReject = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/bast/${idBast}/reject`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-6 space-y-8">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-foreground">Review BAST</h3>
        <p className="text-sm text-muted-foreground">ID: <strong>{bastData.idBast}</strong></p>
        <div className="mt-2">
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
            bastData.status === 'DRAFT' ? 'bg-gray-100 text-gray-800' :
      ...
      <form className="space-y-6">
        ...
        <div className="flex flex-col gap-1">
          <label className="block font-medium">Approver BAST *</label>
          <div className="flex gap-2">
            <input
              type="email"
              value={formData.pejabatApproval}
              onChange={(e) => {
                setFormData({ ...formData, pejabatApproval: e.target.value });
                // Reset status dan error saat input berubah
                setApproverStatus(null);
                setErrors(prev => ({ ...prev, pejabatApproval: undefined }));
              }}
              className={`flex-1 border rounded px-3 py-2 ${errors.pejabatApproval ? 'border-red-500' : ''}`}
