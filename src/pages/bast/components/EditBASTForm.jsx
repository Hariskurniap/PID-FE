// Bast/components/EditBastForm.jsx
...
const EditBastForm = () => {
    ...
    useEffect(() => {
        if (!idBast) {
            alert('ID BAST tidak ditemukan di URL');
            return;
        }
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const bast = await res.json();

                // Map data dari BE ke formData
                setFormData({
                    idBast: bast.idBast || '',
                    invoiceTypeId: bast.invoiceTypeId != null ? bast.invoiceTypeId.toString() : '',
                    nomorPo: bast.nomorPo || '',
                    vendorId: bast.vendorId != null ? bast.vendorId.toString() : '',
                    perihal: bast.perihal || '',
                    nomorKontrak: bast.nomorKontrak || '',
                    reviewerBast: bast.reviewerBast || '',
                    tanggalMulaiKontrak: bast.tanggalMulaiKontrak?.split('T')[0] || '',
                    ...
                    items: bast.detailTransaksi?.map(dt => ({
                        no: dt.no != null ? dt.no : 1,
                        pekerjaan: dt.pekerjaan || '',
                        progress: dt.progress?.toString() || '0',
                        nilaiTagihan: dt.nilaiTagihan?.toString() || '',
                        keterangan: dt.keterangan || ''
                    })) || [{ no: 1, pekerjaan: '', progress: '0', nilaiTagihan: '', keterangan: '' }],
                    dokumenPendukung: bast.dokumenPendukung?.map(doc => ({
                        id: doc.id,
                        nama: doc.nama || '',
                        file: null,
                        filePath: doc.filePath
                    })) || [],
                    statusFaktur: bast.fakturPajak?.statusFaktur || '',
                    nomorFaktur: bast.fakturPajak?.nomorFaktur || '',
                    tanggalFaktur: bast.fakturPajak?.tanggalFaktur?.split('T')[0] || '',
                    jumlahOpp: bast.fakturPajak?.jumlahOpp?.toString() || '',
                    jumlahPpn: bast.fakturPajak?.jumlahPpn?.toString() || '',
                    jumlahPpnBm: bast.fakturPajak?.jumlahPpnBm?.toString() || '',
                    npwpPenjual: bast.fakturPajak?.npwpPenjual || '',
                    namaPenjual: bast.fakturPajak?.namaPenjual || '',
                    alamatPenjual: bast.fakturPajak?.alamatPenjual || '',
                    npwpLawanTransaksi: bast.fakturPajak?.npwpLawanTransaksi || '',
                    namaLawanTransaksi: bast.fakturPajak?.namaLawanTransaksi || '',
                    alamatLawanTransaksi: bast.fakturPajak?.alamatLawanTransaksi || '',
                    berkas: null,
                    detailTransaksi: bast.fakturPajak ? [{
                        id: 1,
                        no: 1,
                        namaBarang: 'Nama Barang Kena Pajak/Jasa Kena Pajak',
                        hargaJual: bast.fakturPajak.hargaJual?.toString() || '0'
                    }] : [{ id: 1, no: 1, namaBarang: 'Nama Barang Kena Pajak/Jasa Kena Pajak', hargaJual: '0' }],
                    creatorBastVendor: bast.creatorBastVendor || '',
                });

                // Load vendor & jenis pengadaan
                ...
                if (bast.vendorId) {
                    const vendorRes = await fetch(
                        `${import.meta.env.VITE_API_BASE_URL}/api/vendors?filter=id eq ${bast.vendorId}`,
                        { headers }
                    );
                    const vendors = await vendorRes.json(); // ← ini array

                    if (vendors.length > 0) {
                        const vendor = vendors[0]; // ambil objek pertama
                        setVendorOptions([{
                            value: vendor.id.toString(),
                            label: vendor.namaVendor,
                        }]);
                        // Opsional: set formData.vendorId jika belum terisi
                ...
                console.error('Gagal load data BAST:', err);
                alert('Gagal memuat data BAST: ' + err.message);
            } finally {
                setLoading(false);
            }
        };
    ...
    const validate = () => {
        ...
        if (!formData.dokumenPendukung.some(d => d.file || d.filePath)) {
    ...
    const prepareFormData = () => {
        ...
        const { dokumenPendukung, items, detailTransaksi, copyKontrak, berkas, ...otherData } = formData;

        // Tambahkan data non-file
        formDataToSend.append('items', JSON.stringify(items));
        formDataToSend.append('detailTransaksi', JSON.stringify(detailTransaksi));
        formDataToSend.append('dokumenPendukung', JSON.stringify(dokumenPendukung));
        ...
        // 🔁 Hanya tambahkan file jika ada (dan bertipe File)
        if (copyKontrak instanceof File) {
            formDataToSend.append('copyKontrak', copyKontrak);
        }

        if (berkas instanceof File) {
            formDataToSend.append('berkas', berkas);
        }

        // Hanya tambahkan dokumen pendukung yang berupa File
        dokumenPendukung.forEach(doc => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // if (submitRes.ok) {
            //     alert('BAST berhasil disubmit untuk review!');
            //     // Redirect ke list
            //     window.location.href = '/bast/list';
            // } else {
            //     alert('Gagal submit: ' + (result.error || 'Server error'));
            // }
    ...
    // Reset Form
    const resetForm = () => {
        if (window.confirm('Yakin ingin reset draft? Semua perubahan akan hilang.')) {
    ...
    return (
        <div className="bg-card border border-border rounded-lg p-6 space-y-6">
            <div className="mb-6">
                <h3 className="text-lg font-semibold text-foreground">Edit BAST</h3>
                <p className="text-sm text-muted-foreground">ID: {formData.idBast}</p>
            </div>
